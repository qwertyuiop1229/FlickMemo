...
function loadLocalNotes() {
    ...
    req.onsuccess = () => {
        ...
        cleanExpiredTrash();
        cleanupEmptyNotes(); // 空のメモの自動クリーンアップ
        renderList(searchInput.value);
...
function renderList(filter = '') {
    ...
    filtered.forEach(n => {
        ...
        delBtn.onclick = (e) => {
            e.stopPropagation(); // liへのクリック連動を防止
            if (currentTab === 'notes') {
                // 通常メモ：確認なしで即時ゴミ箱へ移動
                moveToTrash(n.id);
            } else {
